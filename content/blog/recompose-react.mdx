...
summary: 'ความรู้เบื้องต้นเกี่ยวกับ Functional programing ก่อนใช้งาน Recompose'
image: '/images/recompose-react/banner.jpg'
---
...
<Image
  alt={`choc-factory`}
  src={`/images/recompose-react/choc-factory.png`}
  width={1280}
  height={216}
/>
...
<Image
  alt={`choc-factory`}
  src={`/images/recompose-react/choc-factory-2.png`}
  width={120}
  height={394}
  className="mx-auto"
/>
...
<Image
  alt={`choc-factory`}
  src={`/images/recompose-react/choc-factory-3.png`}
  width={210}
  height={369}
  className="mx-auto"
/>
...
```js
const calculateMeanByNumbers = compose(
  calculateMean,
  toNumbers,
  something2,
  something1
...
// without compose
const toSlug = input =>
  join('-')(map(toLowerCase)(map(removeSpecialChar)(split(/\s/)(input))))
...
```jsx showLineNumbers {19-23} /withState/3-4
import React from 'react'
import withState from './withState'

const FooComponent = props => (
  <div>
    <h1>Clicks: {props.count}</h1>
...
export default withState(
  ...
  'updateCount',
  0
)(withState('text', 'updateText', '')(FooComponent))
```

จะเห็นได้ว่า Component ดูสะอาดตา อ่านง่ายๆ ไม่มี `class` ไม่มี `this` ไม่มี `.bind(this)` แต่…. ยังไม่พอครับ ที่บรรทัด 19 - 23 มีการเรียกฟังก์ชัน `withState` ซ้อนกันอยู่ เดี๋ยวเราจะนำ Compose ที่พูดถึงกันมาก่อนหน้านี้ทำให้การใช้ HoC อ่านง่ายขึ้น
...
const mapProps = mapFn => BaseComponent => props =>
  <BaseComponent {...mapFn(props)} />
